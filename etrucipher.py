from EisensteinElement import EIR
...
log = logging.getLogger("etrucipher")
...
class EtruCipher:
    ...
    def __init__(self, N, p, q):
        ...
        self.p = p
        self.q = q
        self.R_poly = Poly(x ** N - 1, x).set_domain(EIR)
        log.info("ETRU(N={},p={},q={}) initiated".format(N, p, q))

    def generate_random_keys(self):
        g_poly = random_e_poly(self.N, int(math.sqrt(self.q)))
        ...
        while tries > 0 and (self.h_poly is None):
            f_poly = random_e_poly(self.N, self.N // 3, neg_ones_diff=-1)
